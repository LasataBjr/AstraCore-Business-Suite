{{--
    Services — Index Page
    Path: resources/views/admin/services/index.blade.php
...
@section('breadcrumb')
    ...
    <span class="text-slate-500">All Services</span>
@endsection
...
<div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
    <div>
        <h2 class="text-xl font-bold text-slate-800 font-display">Services</h2>
        <p class="mt-0.5 text-sm text-slate-500">Manage the services you offer to clients.</p>
    </div>
    <a
...
<div class="mb-5 grid grid-cols-2 sm:grid-cols-4 gap-3">
    @php
        ...
        $publishedCount = \App\Models\Service::where('status','published')->count();
        $draftCount     = \App\Models\Service::where('status','draft')->count();
        $featuredCount  = \App\Models\Service::where('is_featured', true)->count();
    @endphp
    @foreach ([
        ...
        ['Featured',  $featuredCount,  'bg-indigo-100',  'text-indigo-700',  ''],
    ] as [$label, $count, $bg, $text, $_])
...
<div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
    <form method="GET" action="{{ route('admin.services.index') }}" class="flex flex-1 flex-wrap items-center gap-2">
        ...
        @if (request()->hasAny(['search','status','featured']))
            <a href="{{ route('admin.services.index') }}" class="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-medium text-slate-500 hover:text-slate-700 hover:border-slate-300 transition">
                Clear
            </a>
        @endif
    </form>
        ...
        <table class="min-w-full divide-y divide-slate-100">
            <thead>
                <tr class="bg-slate-50/80">
                    <th scope="col" class="px-5 py-3.5 text-left text-[11px] font-semibold uppercase tracking-wider text-slate-500 w-[30%]">Service</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="group hover:bg-slate-50/60 transition-colors">
                    ...
                    {{-- Service title + image + excerpt --}}
                    <td class="px-5 py-4">
                        <div class="flex items-center gap-3">
                            {{-- Thumbnail --}}
                            <div class="h-11 w-14 flex-shrink-0 overflow-hidden rounded-lg bg-slate-100">
                            ...
                            {{-- Title + slug --}}
                            <div class="min-w-0">
                                <p class="text-sm font-semibold text-slate-700 truncate max-w-[180px]">{{ $service->title }}</p>
                                <p class="mt-0.5 font-mono text-[10px] text-slate-400 truncate max-w-[180px]">{{ $service->slug }}</p>
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="px-5 py-4">
                        ...
                        @else
                            <span class="text-xs text-slate-400">Uncategorised</span>
                        @endif
                    </td>
                    ...
                    <td class="px-5 py-4">
                        @php
                            ...
                            ][$service->status] ?? ['bg-slate-100 text-slate-500', ucfirst($service->status)];
                        @endphp
                    ...
                    <td class="px-5 py-4">
                        <div class="flex items-center justify-end gap-1.5">
                            ...
                            <form
                                ...
                                x-data
                                @submit.prevent="if(confirm('Delete «{{ addslashes($service->title) }}»? This cannot be undone.')) $el.submit()"
                            >
                                @csrf
                                @method('DELETE')
                                <button
                ...
                <tr>
                    <td colspan="7" class="px-5 py-16 text-center">
                        <div class="flex flex-col items-center gap-3 text-slate-400">
                            ...
                            <a href="{{ route('admin.services.create') }}" class="text-xs font-semibold text-indigo-600 hover:text-indigo-800 transition-colors">
    ...
    <div class="border-t border-slate-100 bg-slate-50/60 px-5 py-3.5">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <p class="text-xs text-slate-500">
                Showing <span class="font-medium text-slate-700">{{ $services->firstItem() }}</span>–<span class="font-medium text-slate-700">{{ $services->lastItem() }}</span>
                of <span class="font-medium text-slate-700">{{ $services->total() }}</span> services
            </p>
